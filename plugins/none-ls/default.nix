...
helpers.neovim-plugin.mkNeovimPlugin config {
  ...
  imports =
    let
      # Yes, I did this just to bypass nixfmt...
      oldPluginPath = splitString "." "plugins.null-ls";
      basePluginPath = splitString "." "plugins.none-ls";
    in
    [
      ./servers.nix # TODO: Rename to sources

      # TODO: Deprecations and renames
      (mkRenamedOptionModule oldPluginPath basePluginPath)
      # FIXME: Does this rename work? Matching types, etc?
      (mkRenamedOptionModule (basePluginPath ++ [ "sourcesItems" ]) (
        basePluginPath ++ [ "settings" ] ++ [ "sources" ]
      ))
      # TODO: consider keeping this option? Otherwise offer alternative instructions.
      (mkRemovedOptionModule (basePluginPath ++ [ "enableLspFormat" ]) ''
  ...
  settingsOptions = {
    ...
    # Not using mkListOf because I want `strLua` not `rawLua`
    sources = helpers.mkNullOrOption' {
      # FIXME: need to provide a separate way to "register" custom sources
      # https://github.com/nvimtools/none-ls.nvim/blob/main/doc/MAIN.md#sources
      type = with helpers.nixvimTypes; listOf strLua;
      apply = x: if x == null then null else map (v: if isString v then helpers.mkRaw v else v) x;
      default = null;
      pluginDefault = null;
      # TODO: add note regarding nixvim's builtin sources support (plugins.none-ls.sources)
      description = ''
      ...
      '';
      # TODO: uncomment if all use-cases are covered outside of `settings`
      # - builtin sources
      # - third-party sources
      # - custom sources
      # visible = false;
